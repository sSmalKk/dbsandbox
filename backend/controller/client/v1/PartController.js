...
const softDeletePart = async (req,res) => {
  try {
    ...
    let updatedPart = await deleteDependentService.softDeletePart(query, updateBody);
    if (!updatedPart){
      return res.recordNotFound();
    }
    return res.success({ data:updatedPart });
...
const deletePart = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedPart = await deleteDependentService.countPart(query);
    } else {
      deletedPart = await deleteDependentService.deletePart(query);
    }
    if (!deletedPart){
      return res.recordNotFound();
    }

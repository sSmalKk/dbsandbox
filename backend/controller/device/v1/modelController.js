...
const addModel = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Model,[ 'ip' ],dataToCreate,'INSERT');
...
const bulkInsertModel = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Model,[ 'ip' ],dataToCreate,'BULK_INSERT');
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const updateModel = async (req,res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Model,[ 'ip' ],dataToUpdate,'UPDATE', query);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const bulkUpdateModel = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Model,[ 'ip' ],dataToUpdate,'BULK_UPDATE', filter);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const softDeleteModel = async (req,res) => {
  try {
    ...
    let updatedModel = await deleteDependentService.softDeleteModel(query, updateBody);
    if (!updatedModel){
      return res.recordNotFound();
    }
    return res.success({ data:updatedModel });
...
const deleteModel = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedModel = await deleteDependentService.countModel(query);
    } else {
      deletedModel = await deleteDependentService.deleteModel(query);
    }
    if (!deletedModel){
      return res.recordNotFound();
    }

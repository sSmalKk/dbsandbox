...
const softDeleteTick = async (req,res) => {
  try {
    ...
    let updatedTick = await deleteDependentService.softDeleteTick(query, updateBody);
    if (!updatedTick){
      return res.recordNotFound();
    }
    return res.success({ data:updatedTick });
...
const deleteTick = async (req,res) => {
  try {
    ...
    if (req.body.isWarning) { 
      deletedTick = await deleteDependentService.countTick(query);
    } else {
      deletedTick = await deleteDependentService.deleteTick(query);
    }
    if (!deletedTick){
      return res.recordNotFound();
    }
...
/**
 * @description : startWorldTick 
 * @param {Object} req : request
 * @param {Object} res : response 
 * @return {Object} : response of startWorldTick {status, message, data}
 */
const startWorldTick = async (req,res)=>{
  try {
    // add your code here
    let result = true;
    if (result){

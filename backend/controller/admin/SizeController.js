...
const addSize = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Size,[ 'name', 'realsize' ],dataToCreate,'INSERT');
...
const bulkInsertSize = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Size,[ 'name', 'realsize' ],dataToCreate,'BULK_INSERT');
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const updateSize = async (req,res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Size,[ 'name', 'realsize' ],dataToUpdate,'UPDATE', query);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const bulkUpdateSize = async (req,res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Size,[ 'name', 'realsize' ],dataToUpdate,'BULK_UPDATE', filter);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
